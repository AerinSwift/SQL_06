...
/*
 * Задание 1. Найдите человека, который больше всех общался с нашим пользователем, 
 * иначе, кто написал пользователю наибольшее число сообщений. (можете взять пользователя с любым id).
 * 
 * Получаем id пользователей, который отправил больше всего сообщений пользователю с id=1, сортируем по убыванию
 * и оставляем наибольшее значение
 */
SELECT user_id, sum(message_count) FROM (
	(SELECT from_user_id as user_id, COUNT(*) as message_count FROM messages WHERE to_user_id = '1' GROUP BY from_user_id)
) as my_tmp_table
GROUP by user_id
 ...
 * Получаем количество id пользователей, возраст которых менее 18 лет 
 * (получаемых как разницу между текущим годом и годом рождения)
 * 
 */

select count(*) from (select birthday, posts_likes.user_id from profiles, posts_likes where profiles.user_id = posts_likes.user_id
and (year(now()) - YEAR(birthday)) < '18') as likes_counter;

/*
 * Задание 3. Определить, кто больше поставил лайков (всего) - мужчины или женщины?
 * Соединяем таблицы, исключаем из подсчета незаполненный (х) пол, группируем, 
 * сортируем по убыванию и оставляем первую запись
 */
SELECT COUNT(*) AS likes, gender FROM posts_likes, profiles
WHERE posts_likes.user_id = profiles.user_id and gender <> 'x'
GROUP BY gender
ORDER BY likes desc
LIMIT 1;
